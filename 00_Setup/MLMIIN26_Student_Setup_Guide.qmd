...
**IMPORTANT:** Make a `git pull` from the `upstream` repository to get the latest course files. In your case the upstream repository is the original course repository at [https://github.com/ML-MIC/MLMIIN.git](https://github.com/ML-MIC/MLMIIN.git). When we update the course files (e.g. new notebooks, fixes, etc.) we will push them to that repository, and you will need to pull them into your fork before starting the session. The way to do that depends on how you plan to manage git:
   
   
  + If you are using *GitHub Desktop*, make sure that your fork is selected as *Current Repository*. 
  click on *Repository → Pull* and select *upstream/main* as the branch to pull from.  
   If you are using **VS Code**, click on the **Version Control** icon in the left sidebar, then click on the three dots at the top and select *Pull from...*, then choose *upstream/main*.  
   If you are using **git command line**, run:
   ```bash
   git pull upstream main
   ```
   (You only need to set up the *upstream* remote once; if you haven’t done it yet, see the images below for instructions.)

6. Open a notebook (`.ipynb`). We strongly recommend that you start by making a copy  
